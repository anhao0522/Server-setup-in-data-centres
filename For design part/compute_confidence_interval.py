...
def sim_mmm_func(mode,m,setup_time,delayedoff_time):
    ...
    while True:
        ...
        if next_event_type == 1: #arrival
            ...
            response_time.append(master_clock - arrival_time_next_departure[first_departure_server])
    ...
    return sum(response_time[w:])/(num_customer_served-w)
...
for i in range(n):
    random.seed(i)
    mrt1.append(sim_mmm_func('random',5,5,0.1))
    mrt2.append(sim_mmm_func('random',5,5,9.8))
mrt1=np.array(mrt1)
mrt2=np.array(mrt2)
mean_diff=mrt1-mrt2
df=n-1
sc=np.std(mean_diff,ddof=1)/np.sqrt(n)
a,b = ss.t.interval(0.95, df=df, loc=np.mean(mean_diff), scale=sc)
print(a,b)